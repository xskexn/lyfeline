...
class SharedCareStorage {
  final String aspectId;

  SharedCareStorage._(this.aspectId);

  static Future<SharedCareStorage> init(String aspectId) async {
    return SharedCareStorage._(aspectId);
  }
  ...
  Future<void> saveData(List<Atom> atoms, int points, List<Map<String, dynamic>> careRecords) async {
    ...
    await prefs.setString(recordsKey, jsonEncode(careRecords));
    
    await prefs.setString(lastResetKey, DateTime.now().toIso8601String());
  }

  int _calculateLevel(int totalDays) {
  ...
  Future<void> finaliseDay(int dailyPoints, List<Map<String, dynamic>> careRecords) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    final newLevel = _calculateLevel(newTotalDays);
  ...
  Future<(List<Atom>, int, DateTime?, List<Map<String, dynamic>>, int, double, int, int)> loadData(List<Atom> defaultAtoms) async {
    ...
    if (recordsJson != null) {
      try {
        ...
        careRecords = decoded.cast<Map<String, dynamic>>();
      } catch (e) {
        print("Error loading care records: $e");
      }
    }
    ...
    return (atoms, points, lastReset, careRecords, level, progress, totalDays, totalPoints);
  }
